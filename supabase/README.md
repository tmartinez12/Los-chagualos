...
| 5 | `migracion-pesajes.sql` | Crea la tabla `pesajes` (historial de peso con fecha — acción "⚖️ Pesaje" de la ficha) y añade `animales.peso_nacer` (peso al nacer manual para animales sin parto registrado; recrea `v_animales` para exponerlo). Actualiza `restaurar_respaldo()` para incluir `pesajes` y `vacunaciones_animales` (faltaba en la versión instalada). | **pendiente** (jul 2026) |
