...
- **B8 · Edge function `login-pin` huérfana con debilidades** (`supabase/edge-functions-archivadas/login-pin/index.ts`): JWT_SECRET cae a la service_role key, rate-limit burlable por `device_id` del cliente, mensajes que permiten enumerar usuarios. → archivar hasta que vuelva el login; corregir antes de desplegar.
